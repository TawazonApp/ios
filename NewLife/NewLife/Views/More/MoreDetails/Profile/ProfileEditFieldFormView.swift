//
//  ProfileEditFieldFormView.swift
//  NewLife
//
...
protocol ProfileEditFieldFormViewDelegate: class {
    func showErrorMessageView(title: String?, message: String)
    func submit()
}

class ProfileEditFieldFormView: UIView {
    ...
    weak var delegate: ProfileEditFieldFormViewDelegate?
    
    let cellHeight: CGFloat = (UIScreen.main.bounds.height <= 568.0) ? 50 : 60
    
    var data: ProfileFieldFormVM! {
        didSet {
            reloadData()
        }
    }
...
extension ProfileEditFieldFormView: UITableViewDelegate, UITableViewDataSource {
...
extension ProfileEditFieldFormView: MembershipTextFieldCellDelegate {
