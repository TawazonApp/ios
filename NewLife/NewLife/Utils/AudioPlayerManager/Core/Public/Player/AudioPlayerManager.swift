...
open class AudioPlayerManager: NSObject {
	...
	public override init() {
		...
		NotificationCenter.default.addObserver(self, selector: #selector(AudioPlayerManager.trackDidFinishPlaying(_:)), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: nil)
	}
	...
	open func stop(clearQueue: Bool = false) {
        if let _currentTrack = self.currentTrack,
           let session = currentSession {
            HomeServiceFactory.service(
            ).trackSessionDidEnd(sessionId: session.id, duration: TimeInterval(_currentTrack.currentTimeInSeconds())) { (_) in
            }
        }
    ...
    @objc open func trackDidFinishPlaying(_ notification: Notification) {
        guard let notificationPlayerItem =  notification.object as? AVPlayerItem, self.player?.currentItem == notificationPlayerItem else {
            return
        }
	...
	open func isPlaying() -> Bool {
		...
		return _player.rate > 0 && (_player.currentItem != nil)
	}
	...
	fileprivate func restartCurrentTrack() {
		...
		if let _currentTrack  = self.queue.currentTrack {
			...
			if let _playerItem = _currentTrack.getPlayerItem() {
                ...
                if #available(iOS 10.0, *) {
                    _playerItem.preferredForwardBufferDuration = TimeInterval(0)
                } 
			}
		}
	}
    ...
    @objc func callPlaybackTimeChangeCallbacks() {
		...
		if let _currentTrack = self.currentTrack {
            ...
            if currentSession?.isLock ?? false {
                let seconds = _currentTrack.currentTimeInSeconds()
