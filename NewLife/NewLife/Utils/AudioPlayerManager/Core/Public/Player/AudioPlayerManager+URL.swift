...
extension AudioPlayerManager {
	...
	// MARK: - Play
	 ...
	 func play(urlString string: String, session: SessionModel?) {
        self.play(url: URL(string: string), session: session)
	}
	...
	public func play(urlStrings strings: [String], at startIndex: Int) {
     ...
     func play(url urlToPlay: URL?, session: SessionModel?) {
    ...
    public func isCurrentTrack(url urlToCheck: URL?) -> Bool {
        return (self.currentTrack?.identifier() == urlToCheck?.absoluteString)
    }
    
    public func getCurrentTrack() -> AudioTrack? {
        return currentTrack
    }
}
